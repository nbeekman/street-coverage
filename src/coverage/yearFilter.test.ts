...
describe('maskFor', () => {
  ...
  it('sets one bit per year index', () => {
    expect(maskFor(Y2021)).toBe(0b001)
    expect(maskFor(Y2023)).toBe(0b100)
  })
})
...
describe('runCounts', () => {
  it('never counts an unridden run, filter or not', () => {
    ...
    expect(runCounts(0, bit(Y2022), maskFor(Y2022))).toBe(false)
  })
  ...
  it('counts a run only in the years it was ridden', () => {
    ...
    expect(runCounts(1, years, maskFor(Y2021))).toBe(true)
    expect(runCounts(1, years, maskFor(Y2022))).toBe(false)
    expect(runCounts(1, years, maskFor(Y2023))).toBe(true)
  })
})
...
describe('filteredTotals', () => {
  ...
  it('sums only the selected year', () => {
    expect(filteredTotals([denver], Y2021).byRegion.get('denver')).toBeCloseTo(150, 5)
    expect(filteredTotals([denver], Y2022).byRegion.get('denver')).toBeCloseTo(250, 5)
  })
  ...
  it('counts a stretch ridden in two years under each of them', () => {
    ...
    const a = filteredTotals([denver], Y2021).byRegion.get('denver')!
    const b = filteredTotals([denver], Y2022).byRegion.get('denver')!
  ...
  it('returns zero for a year with no riding', () => {
    expect(filteredTotals([denver], Y2023).byRegion.get('denver')).toBe(0)
    expect(filteredTotals([denver], Y2023).coreCoveredMeters).toBe(0)
  })
